...
function MiniMap({ lat, lng, title }) {
  ...
  useEffect(() => {
    ...
    map.on("style.load", () => {
      ...
      el.innerHTML = `<div style="width:20px;height:20px;border-radius:50%;background:#FF3B3B;border:3px solid #fff;box-shadow:0 0 10px #FF3B3B88;"></div>`;
  ...
  if (!MAPBOX_TOKEN) {
    return (
      <div style={{ height: 120, background: "#050e1a", borderRadius: 10, border: "1px solid #1e293b", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <span style={{ fontSize: 11, color: "#334155" }}>Mapa indisponível — configure VITE_MAPBOX_TOKEN</span>
      </div>
    );
  }
    ...
    <div style={{ padding: "28px 32px" }}>
      ...
      <div style={{ display: "flex", background: "#0a1628", borderRadius: 12, marginBottom: 16, overflow: "hidden", border: "1px solid #0f2040" }}>
        {[
          ...
          <button key={a.id} onClick={() => setAba(a.id)} style={{ flex: 1, padding: "12px", border: "none", background: "transparent", cursor: "pointer", color: aba === a.id ? "#ff3b3b" : "#475569", fontSize: 12, fontWeight: 700, borderBottom: aba === a.id ? "2px solid #ff3b3b" : "2px solid transparent", fontFamily: "monospace" }}>
            {a.label}
          </button>
        ))}
      </div>
      ...
      {aba === "necessidades" && (
        <div style={{ display: "grid", gap: 10 }}>
          {demandas.map(d => (
            <div key={d.id} style={{ background: "#0a1628", border: `1px solid ${d.priority >= 5 ? "#ef444430" : "#0f2040"}`, borderRadius: 12, padding: "16px 20px", display: "grid", gridTemplateColumns: "auto 1fr auto", gap: 16, alignItems: "center" }}>
              <div style={{ width: 42, height: 42, borderRadius: 10, background: `${categoryColors[d.category]}15`, border: `1px solid ${categoryColors[d.category]}30`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, color: categoryColors[d.category], fontWeight: 700 }}>
      ...
      {aba === "voluntarios" && (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14 }}>
          ...
          {voluntarios.map(v => (
            <div key={v.id} style={{ background: "#0a1628", border: `1px solid ${v.available ? "#10b98130" : "#0f2040"}`, borderRadius: 12, padding: "16px", display: "flex", alignItems: "center", gap: 12 }}>
              <div style={{ width: 38, height: 38, borderRadius: "50%", background: v.available ? "#10b98120" : "#1e293b", border: `2px solid ${v.available ? "#10b981" : "#334155"}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16 }}>
              ...
              <div style={{ width: 8, height: 8, borderRadius: "50%", background: v.available ? "#10b981" : "#475569", boxShadow: v.available ? "0 0 6px #10b981" : "none" }} />
            </div>
          ))}
        </div>
      )}
